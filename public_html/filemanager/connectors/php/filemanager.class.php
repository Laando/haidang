...
class Filemanager {
	...
	public function add() {
        ...
        $this->waterMarkImage($_FILES['newfile']['name'],$current_path);
		echo '<textarea>' . json_encode($response) . '</textarea>';
...
public function expandPath($path, $clean = false)
{
    ...
    public function waterMarkImage($link,$path){
        $stamp = imagecreatefrompng('/home/haidangv/public_html/filemanager/userfiles/img/tem_logo.png');
        $sx = imagesx($stamp);
        $sy = imagesy($stamp);
        $im = imagecreatefromjpeg($path.$link);
        $result = imagecopy($im, $stamp, (imagesx($im) - $sx)/2, (imagesy($im) - $sy)/2, 0, 0, imagesx($stamp), imagesy($stamp));
        imagejpeg ( $im,$path.$link );
        imagedestroy($im);
        //dd('result : '.$result);
    }
}
