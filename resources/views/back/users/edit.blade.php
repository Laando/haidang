...
@section('main')
 ...
 <!-- Entête de page -->
  @include('back.partials.entete', ['title' => trans('back/users.dashboard'), 'icone' => 'user', 'fil' => link_to('user', trans('back/users.Users')) . ' / ' . trans('back/users.edition')])
	...
	<div class="col-sm-12">
		{!! Form::model($user, ['files' => true ,'route' => ['user.update', $user->id], 'method' => 'put', 'class' => 'form-horizontal panel']) !!}
            ...
            {!! Form::selection('member_card_type',  $select_card_type,$user->member_card_type, 'Loại Member Card') !!}
            <div class="form-group">
                <label for="dob" class="control-label">Năm sinh</label>
                {!! Form::text('dob', date_format(date_create($user->dob),'d/m/Y') ,array('class'=>'form-control datepicker')) !!}
            </div>
            ...
            <div class="form-group col-lg-12 avatar">
                {!! Form::control('file', 0, 'avatar', $errors, 'Hình đại diện') !!}
                <input type="hidden" name="old-avatar" value="{{ $user->avatar }}"/>
                @if($user->avatar!= '')
                        <div>{!! HTML::image('image/avatar/'.$user->avatar,'',array('class'=>'img-responsive')) !!}</div>
                @endif
            </div>
            {!! Form::submit('Cập nhật') !!}
		{!! Form::close() !!}
        <hr>
        {!! Form::open(['url' => 'user/AdminChangePasswordUser/'.$user->id, 'method' => 'put', 'class' => 'form-horizontal panel']) !!}
