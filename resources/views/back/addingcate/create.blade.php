...
@section('main')
 ...
 <!-- Entête de page -->
  @include('back.partials.entete', ['title' => trans('back/addingcates.dashboard'), 'icone' => 'send', 'fil' => link_to('addingcate', trans('back/addingcates.addingcates')) . ' / ' . trans('back/addingcates.creation')])
	...
	<div class="col-sm-12">
		{!! Form::open(['url' => 'addingcate', 'method' => 'post', 'class' => 'form-horizontal panel']) !!}
			{!! Form::control('text', 0, 'title', $errors, trans('back/addingcates.title')) !!}
            {!! Form::control('number', 0, 'seat', $errors, trans('back/addingcates.seat')) !!}
             Bắt buộc <input value="1" type="checkbox" name="isForced">
			{!! Form::submit('Tạo mới') !!}
		{!! Form::close() !!}
	</div>
