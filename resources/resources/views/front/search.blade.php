...
<section class="exploore page-title">
    <div class="container">
        <div class="page-title-wrapper">
            ...
            @else
                @if(isset($destinationpoint))
                    @if(str_contains(Request::path(),'chu-de-tour'))
                        {!! Breadcrumbs::render('chu-de',$destinationpoint) !!}
                    @else
                        {!! Breadcrumbs::render('diem-den',$destinationpoint) !!}
                    @endif
                @else
                    @if($isOutbound==1))
                        {!! Breadcrumbs::render('tour-nuoc-ngoai') !!}
                    @else
                        {!! Breadcrumbs::render('tour-trong-nuoc') !!}
                    @endif
                @endif
            @endif
            <!--=== End Breadcrumbs ===-->
