    ...
    <div class="container">
        <div class="page-title-wrapper">
            <div class="page-title-content">
                ...
                @if(isset($category))
                    <h2 class="captions">{{$category->title}}</h2>
                @else
                    <h2 class="captions">Bài viết</h2>
                @endif
            </div>
        </div>
    </div>
                        ...
                        <div class="tours-list">
                            <div class="row tourcate-main">
                                <?php foreach($blogs as $blog) : ?>
                                    <div class="col-md-12 blog-post post-220 post type-post status-publish format-image has-post-thumbnail hentry category-discover category-the-world category-travel tag-adventure tag-europe tag-explore post_format-post-format-image">
                                        <div class="blog-image">
                                            <?php 
                                                $arrimg = explode(';',$blog->images);
                                            ?>
                                            <a href="/tin-tuc/detail/<?= $blog->slug ?>" class="link">
                                                <img src="{{asset('image/'.$arrimg[0])}}" class="img-responsive" alt="{!! $blog->title !!}">
                                            </a>
                                        </div>
                                        <div class="blog-content">
                                            ...
                                            <div class="col-xs-10 content-wrapper">
                                                <a href="/tin-tuc/detail/<?= $blog->slug ?>" class=" heading">{!! $blog->title !!}</a>
                                                <h5 class="meta-info">
                                                    <span>Posted by:</span> 
                                                    <a href="/tin-tuc/detail/<?= $blog->slug ?>" class="">
                                                        <span>Admin</span>
                                                    </a> 
                                                    <span class="sep">/</span>
                                                    <span class="view-count fa-custom"><?= $blog->view ?></span>
                                                    <span class="comment-count fa-custom">
                                                        <a href="/tin-tuc/detail/<?= $blog->slug ?>">0</a>
                                                    </span>
                                                </h5>
                                                <div class="preview">
                                                    <?= $blog->description ?>
                                                </div>
                                                <a href="/tin-tuc/detail/<?= $blog->slug ?>" class="btn btn-gray btn-fit btn-capitalize">Xem thêm</a>
                                                </div>
                                            </div>
                                        </div>
                                    <?php //echo $blog;die; ?>
