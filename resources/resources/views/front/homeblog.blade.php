                        ...
                        <div class="form-group mx-1 wline-sm-100 mb-2" style="width:19%">
                            <select name="category" class="form-control rounded-0" style="width:100%">
                                ...
                                @foreach($subjectblogs_blog as $key=>$dp)
                                    <option {{ \Illuminate\Support\Facades\Input::get('category')== strtolower($key) || ($subjectblog_blog && strtolower($subjectblog_blog->slug) == strtolower($key)) ? 'selected':''}} value="{{ $key }}">{{ $dp }}</option>
                                @endforeach
                            </select>
                        </div>
