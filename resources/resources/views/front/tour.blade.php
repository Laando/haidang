            ...
            <div>
                ...
                <div class="row mb-md-4 mb-2 mx-md-auto mx-0">
                    ...
                    <div class="col-md-6 col-12 rounded-0 pr-0 pl-md-2 pl-0">
                        <p class="py-2 text-center font-weight-bold bg-d4c13b mb-0">GIÁ ĐANG KHUYẾN MÃI <span class="gold-title">{{ $isEvent && $isGolden ?'TOUR GIỜ VÀNG':'' }}</span></p>
                        <div class="bg-f9e767 py-2">
                            ...
                            @if($isEvent && $isGolden)
                            <p class="font-weight-bold mb-0 text-center text-danger"><span class="small text-dark pr-2">Tour đang trong giờ vàng vui lòng chọn ngày khởi hành để xem giá giảm</span></span></p>
                            @endif
                        </div>
                        <?php
                        $str_sd = array();
...
@section('ready')
    ...
    @foreach($startdates as $sd)
        <?php $sd->addings = '' ?>
        @if($sd->isEvent == 1)
            inEventStartDates = [ ...inEventStartDates , JSON.parse('{!! $sd->toJson() !!}') ];
        @endif
